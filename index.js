...
switch (process.argv[2]) {
  ...
  case "use":
    ...
    fs.lstat(`${__dirname}/configs/${configName}`, (err, stats) => {
      if (err || !stats.isFile()) {
        console.log(
          "Not a valid config. See 'gitsc list' for available configs"
        );
        process.exit(-1);
      }
  ...
  case "help":
    ...
    console.log(
      "\tgitsc help\r\t\t\t\tShow this help message (alias: gitsc h)"
    );
    console.log(
      "\tgitsc setup\r\t\t\t\tSetup GitSC. Should only be used once."
    );
    console.log(
